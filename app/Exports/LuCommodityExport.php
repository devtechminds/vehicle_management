...
class LuCommodityExport implements FromCollection ,WithMapping, WithHeadings
{
    protected $request;
    ...
    */
    public function collection()
    {
       $request_data = $this->request;
            ...
            if(isset($request_data['commodity'])){
             $loading_gate_entry->whereHas('getLuGateEntry', function($q) use($request_data){
                $q->where('commodity','=',$request_data['commodity']);
            });

             }

            if(isset($request_data['material'])){

                $loading_gate_entry->where('material','=',$request_data['material']);
