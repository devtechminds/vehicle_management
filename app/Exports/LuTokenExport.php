...
class LuTokenExport implements FromCollection ,WithMapping, WithHeadings
{
    protected $request;
    ...
    */
    public function collection()
    {
       $request_data = $this->request;
            ...
            if(isset($request_data['token_no'])){

             $loading_gate_entry->whereHas('getLuGateEntry', function($q) use($request_data){
                $q->where('ref_no','=',$request_data['token_no']);
            });

             }


            if(isset($request_data['wb_ticket_no'])){

                $loading_gate_entry->whereHas('getLuGateEntry.getLuWeightBridge', function($q) use($request_data){
                   $q->where('wb_ticket_no','=',$request_data['wb_ticket_no']);
               });
