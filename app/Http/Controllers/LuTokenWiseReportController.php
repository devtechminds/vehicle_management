...
use App\Exports\LuTokenExport;

class LuTokenWiseReportController extends Controller
{
    public function luTokenReport(Request $request)
    {
        ...
        if($request->ajax()){
           ...
           $loading_gate_entry = LuCommodityDetail::with('getLuGateEntry.getCustomer','getLuGateEntry.getCommodity','getLuGateEntry.getTransporter','getLuGateEntry.getLuWeightBridge','getLuGateEntry','getMaterial','getUOM');
            ...
            if(isset($request->token_no)){
               
             $loading_gate_entry->whereHas('getLuGateEntry', function($q) use($request){
                $q->where('ref_no','=', $request->token_no);
             });

             }   
             
            if(isset($request->wb_ticket_no)){
               
                $loading_gate_entry->whereHas('getLuGateEntry.getLuWeightBridge', function($q) use($request){
                   $q->where('wb_ticket_no','=', $request->wb_ticket_no);
                });
            ...
            return datatables()->of($loading_gate_entry_list)
                ...
                return view('lu_reports.token_report');
            }
    }

    public function tokenDownload(Request $request){
       
        return Excel::download(new LuTokenExport($request->all()), 'Tokenwise.xlsx');
