...
use App\Exports\LuCommodityExport;

class LuCommodityWiseReportController extends Controller
{
    public function luCommodityReport(Request $request)
    {
        ...
        if($request->ajax()){
           ...
           $loading_gate_entry = LuCommodityDetail::with('getLuGateEntry.getCustomer','getLuGateEntry.getCommodity','getLuGateEntry.getTransporter','getLuGateEntry.getLuWeightBridge','getLuGateEntry','getMaterial','getUOM');
            ...
            if(isset($request->material)){
                ...
                $loading_gate_entry->where('material','=',$request->material);
            ...
            return datatables()->of($loading_gate_entry_list)
                ...
                return view('lu_reports.commodity_report')->with('commoditys',$commoditys)->with('materials',$materials);
            }
    }

    public function commodityDownload(Request $request){
       
        return Excel::download(new LuCommodityExport($request->all()), 'Customer.xlsx');
